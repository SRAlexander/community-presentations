...
@inject
def getTest(id,_testsService: TestsService = Provide[Container._testsService]):
    """
    ...
    if queryResult == None:
        statusCode = 404
    response = make_response(json.dumps(queryResult, cls=AlchemyEncoder), statusCode)
    response.headers["Content-Type"] = "application/json"
    return response

@testsControllerBlueprint.route('name/<name>', methods=['GET'])
@inject
def getTestByName(name, _testsService: TestsService = Provide[Container._testsService]):
    """
    An endpoint which returns one test definition record by name
    ---  
    tags:
        - Tests Controller
    parameters:
    ...
    responses:
        ...
        500:
            description: Internal Server Error
    """
    queryResult = _testsService.getTestByName(name)
    statusCode = 200
    if queryResult == None:
        statusCode = 404
    response = make_response(json.dumps(queryResult, cls=AlchemyEncoder), statusCode)
...
def getTests(_testsService: TestsService = Provide[Container._testsService]):
    """
    An endpoint which returns all test definitions
    ---  
    tags:
        - Tests Controller
    responses:
...
def addTest(_testsService: TestsService = Provide[Container._testsService]):
    """
    Adds a new test definition record to the DB
    ---  
    tags:
        - Tests Controller
    parameters:
